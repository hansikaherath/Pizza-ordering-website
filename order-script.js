...
function verify() {
  ...
  if (cardactive.checked) {
    if (cardname == "") error.push("Error: Card name must not be empty!!!");
    if (cardnumber == "") error.push("Error: Card Number must not be empty!!!");
    if (ccv == "") error.push("Error: CCV must not be empty!!!");
  }
